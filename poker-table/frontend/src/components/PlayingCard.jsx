...
const PlayingCard = ({ rank, suit, faceDown = false, size = "md", delay = 0 }) => {
  ...
  return (
    <div
      className={`${sizeClasses[size]} rounded-md bg-white shadow-card relative flex flex-col items-center justify-between p-1 animate-deal select-none`}
      style={{
    ...
    >
      {/* Top-left corner */}
      <div
        className={`self-start leading-none ${textSizes[size]} font-bold font-mono`}
        style={{ color: suitInfo.color }}
      >
        <div>{r}</div>
        <div>{suitInfo.symbol}</div>
      </div>
      ...
      <div
        className={`self-end leading-none ${textSizes[size]} font-bold font-mono rotate-180`}
        style={{ color: suitInfo.color }}
      >
        <div>{r}</div>
        <div>{suitInfo.symbol}</div>
